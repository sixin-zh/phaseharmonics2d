...
- `fx` : final value a
- stop : to stop if there is no success at all

(Koray Kavukcuoglu, 2012)
--]]
function optim.cg2(opfunc, x, config, state)
   -- parameters
   ...
   local targetValue = config.targetValue or 0
   local red = 1
   ...
   while i < math.abs(maxEval) do
	   ...
	   -- if fx is small enough, then early stop
	   if targetValue ~= 0 then
		   if fx < targetValue then
			   break
		   end
	   end
      ...
      if success == 1 then
		 ...
		 stop = 0
		 if verbose == 2 then
			 print('succeed at i='..i)
		 end
      else
		  if verbose == 1 and ls_failed then
			  print('ls failed at i='..i)
		  end
		  x:copy(x0)
		  f1 = f0
		  df1:copy(df0)
